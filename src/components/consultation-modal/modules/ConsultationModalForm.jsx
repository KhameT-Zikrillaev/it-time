...
const ConsultationModalForm = ({ formData, setFormData, onClose, setShowSuccessModal }) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <input
          ...
          placeholder={t('consultationModal.form.name')}
        ...
        {errors.name && (
          <p className="mt-1 text-sm text-red-500">
            {t(`consultationModal.errors.name.${errors.name}`)}
          </p>
        )}
      </div>
      ...
      <div>
        ...
        >
          <option value="">{t('consultationModal.form.selectCourse')}</option>
          {courses.map((course) => (
        ...
        {errors.course && (
          <p className="mt-1 text-sm text-red-500">
            {t(`consultationModal.errors.course.${errors.course}`)}
          </p>
        )}
      </div>
      ...
      <div>
        <input
          ...
          onChange={(e) => handleFormChange(e, setFormData)}
          placeholder={t('consultationModal.form.phone')}
          pattern="[+][0-9]*"
          inputMode="numeric"
        ...
        {errors.phone && (
          <p className="mt-1 text-sm text-red-500">
            {t(`consultationModal.errors.phone.${errors.phone}`)}
          </p>
        )}
      </div>
